...
unsigned char logsyn[] = { OPEND, STRING, OPEND, SP, STRING, OPEND, SP, STRING, END };
unsigned char dirsyn[] = { OPTIONS, NL, STRING, NL, END };
unsigned char compsyn[] = { OPTIONS, NL, STRING, NL, STRING, NL, END };
unsigned char changesyn[] = { NL, FHSKIP, 2, STRING, NL, OPEND, PNAME, SP, PVALUE, NL, REPEAT };
...
parseargs(unsigned char *args, struct command *c, struct transaction *t)
{
	...
	for (syntax = c->c_syntax; *syntax != END;) {
		...
		case NUMBER:
			...
			continue;
		case STRING:
			if ((errcode = string(&args, syntax, (unsigned char **) &a->a_strings[nstrings++])))
				goto synerr;
			continue;
		case PNAME:
...
respond(struct transaction *t, char *results)
{
	...
	sprintf(data, "%s %s %s%s%s", t->t_tid, t->t_fh != FNULL ? t->t_fh->f_name : "", t->t_command->c_name, results != NOSTR ? " " : "", results != NOSTR ? results : "");
...
error(struct transaction *t, char *fh, int code)
{
	...
	sprintf((char *) data, "%s %s ERROR %s %c %s", t->t_tid, fh, e->e_code, errtype ? errtype : E_COMMAND, errstring ? errstring : e->e_string);
...
login(struct transaction *t)
{
	...
	for (host_name = 0; host_name < NUM_PRIVILEGED_HOSTS; host_name++) {
		if (strcmp(privileged_hosts[host_name], chaos_name(mylogin.cl_haddr)) == 0) {
			privileged_host = 1;
			break;
		}
	}
	...
	if ((name = a->a_strings[0]) == NOSTR) {
		...
		sprintf(response, "%s %s/%c%s%c", p->pw_name, p->pw_dir, CHNL, fullname(p), CHNL);
...
fileopen(struct transaction *t)
{
	...
	if (protocol > 0) {
		...
		if (tm->tm_year > 99)
			tm->tm_year = 99;
		sprintf(response, "%02d/%02d/%02d %02d:%02d:%02d %ld %s%s%s%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, (long) nbytes, qfasl, foptions & O_DEFAULT ? " " : "", foptions & O_DEFAULT ? (foptions & O_CHARACTER ? TRUE : FALSE) : "", CHNL, realname, CHNL);
#else
		if (tm->tm_year > 99)
			tm->tm_year += 1900;
		sprintf(response, "%02d/%02d/%04d %02d:%02d:%02d %ld %s%s%s%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, (long) nbytes, qfasl, foptions & O_DEFAULT ? " " : "", foptions & O_DEFAULT ? (foptions & O_CHARACTER ? TRUE : FALSE) : "", CHNL, realname, CHNL);
#endif
	} else {
		if (tm->tm_year > 99)
			tm->tm_year = 99;
		sprintf(response, "%d %02d/%02d/%02d %02d:%02d:%02d %ld %s%c%s%c", -1, tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, (long) nbytes, qfasl, CHNL, realname, CHNL);
	}
...
struct xfer *
makexfer(struct transaction *t, long foptions)
{
	struct xfer *x;
...
xcommand(struct transaction *t)
{
	...
	if ((f = t->t_fh) == FNULL || (x = f->f_xfer) == XNULL) {
		...
		xqueue(t, x);
		// We may have a packet stuck waiting to transmit, force the transmit window to close and tickle the connection.
...
xclose(struct xfer *ax)
{
	...
	if (errcode == 0) {
		...
		if (x->x_options & O_PRESERVE || x->x_flags & (X_ATIME | X_MTIME)) {
			...
			timep->actime = (x->x_options & O_PRESERVE || x->x_flags & X_ATIME) ? x->x_atime : sbuf.st_atime;
			timep->modtime = (x->x_options & O_PRESERVE || x->x_flags & X_MTIME) ? x->x_mtime : sbuf.st_mtime;
		...
		if (protocol > 0) {
			...
			if (tm->tm_year > 99)
				tm->tm_year += 1900;
			sprintf(response, "%02d/%02d/%04d %02d:%02d:%02d %ld%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, sbuf.st_size, CHNL, x->x_realname, CHNL);
#else
			if (tm->tm_year > 99)
				tm->tm_year = 99;
...
filepos(struct xfer *x, struct transaction *t)
{
	if ((x->x_options & O_READ) == 0) {
		...
		if (new < 0 || new > size) {
			...
			if (log_verbose) {
				chaosfile_log(LOG_INFO, "pid: %ld, x: %X, fd: %ld, size: %ld, " "old: %ld, new: %ld, pos: %ld\n", getpid(), x, x->x_fd, size, old, new, tell(x->x_fd));
			}
		}
	}
}
...
complete(struct transaction *t)
{
	...
	if (dfile[0] != '/') {
		...
		if (ifile[0] != '/') {
			if ((adir = malloc((unsigned) (strlen(ddir) + strlen(ifile) + 2))) == NOSTR)
				fatal(NOMEM);
			strcpy(adir, ddir);
			strcat(adir, "/");
			strcat(adir, ifile);
...
getmdate(struct stat *s, char *cp)
{
	...
	if (tm->tm_year > 99)
		tm->tm_year += 1900;
	sprintf(cp, "%02d/%02d/%04d %02d:%02d:%02d", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec);
#endif
	while (*cp)
		cp++;
	return cp;
...
parsetime(char *cp, time_t *t)
{
	...
	if (!(cp = tnum(cp, '/', &month)) || !(cp = tnum(cp, '/', &day)) || !(cp = tnum(cp, ' ', &year)) || !(cp = tnum(cp, ':', &hour)) || !(cp = tnum(cp, ':', &minute)) || !(cp = tnum(cp, '\0', &second)) || month < 1 || month > 12 || day < 1 || day > 31 || year < 70 || hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 || second > 59)
		return 1;
	if (year < 100)
		year += 1900;
	*t = 0;
...
#if defined(MAP_SITE_TREE_DIRECTORY)
		if (treeroot == NULL || path[0] != '/' || tolower(path[1]) != 't' || tolower(path[2]) != 'r' || tolower(path[3]) != 'e' || tolower(path[4]) != 'e' || (path[5] != '/' && path[5] != '\0'))
			wd = "";
		else {
...
fherror(struct file_handle *f, int code, int type, char *message)
{
	...
	packet.cp_op = ASYNOP;
	sprintf(packet.cp_data, "TIDNO %s ERROR %s %c %s", f->f_name, e->e_code, type, message ? message : e->e_string);
	...
	case O_DIRECTORY:
		...
		while (x->x_room != 0) {
			if (x->x_left == 0) {
				...
				if (log_verbose) {
					chaosfile_log(LOG_INFO, "Before read pos: %ld\n", tell(x->x_fd));
				}
				n = x->x_options & O_DIRECTORY ? dirread(x) : x->x_options & O_PROPERTIES ? propread(x) : read(x->x_fd, x->x_bbuf, FSBSIZE);

				if (log_verbose) {
					chaosfile_log(LOG_INFO, "pid: %ld, x: %X, fd: %ld, " "Read: %ld\n", getpid(), x, x->x_fd, n);
				}
				switch (n) {
				case 0:
					if (xpwrite(x) < 0)
